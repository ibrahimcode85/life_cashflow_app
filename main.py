...
if __name__ == "__main__":
    ...
    mort_tab        = prj.generate_mortality_rate_table (age_col      , GENDER     , MORT_TABLE, is_cover_col)
    ...
    unit_cf_pp_proj = prj.generate_unit_fund_cashflow_table  (CONT_Y, is_cover_col, pol_year_col, WAKALAH_TABLE, SUM_ASSD, mort_tab, COI_LOADING, rfr_proj_tab, WAKALAH_FMC)
    risk_cf_pp_proj = prj.generate_risk_fund_cashflows_table (unit_cf_pp_proj, mort_tab, rfr_proj_tab, SUM_ASSD, SURPLUS_SHARE_SHF, SURPLUS_SHARE_PH)
    shf_cf_pp_proj  = prj.generate_shf_cashflows             (unit_cf_pp_proj, rfr_proj_tab, risk_cf_pp_proj, EXP_CONT_Y, EXP_FUND_Y)

    # Project inforce cashflows
    unit_cf_if_proj = prj.generate_cashflow_if_df (unit_cf_pp_proj, pol_count_proj, is_unit_fund = True)
